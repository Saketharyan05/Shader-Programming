...
#if ENABLE_PART2_3D
#if ENABLE_INFINITY_TUNNEL
    // Part 2 overlay: after the black hole, the viewer is pulled into an infinite 3D raymarched tunnel.
    float tunnelAmount = smoothstep(56.0, 61.0, time) * (1.0 - smoothstep(72.0, 78.0, time));
    vec3 tunnelColor = sceneInfinityTunnel3D(suv, time - 56.0);
    color = mix(color, tunnelColor, tunnelAmount * 0.88);
#endif

#if ENABLE_REFLECTIVE_CORE
    // Part 2 overlay: a classical ray-traced reflective 3D life core during rebirth.
    float core3DAmount = smoothstep(84.0, 89.0, time) * (1.0 - smoothstep(96.0, 99.0, time));
    color += sceneReflectiveLifeCore3D(suv, time - 82.0) * core3DAmount * 0.85;
#endif
#endif
